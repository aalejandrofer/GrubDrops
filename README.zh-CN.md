...
| **CPU 架构** | 任意 — `amd64` + `arm64` | `amd64` 主机（见 ARM 说明） |

Twitch 走直连 HTTP，所以一个普通的 Go 二进制文件在任何地方都能挖它 —— 包括树莓派（Raspberry Pi），无需 Docker。**Kick 的观看时长需要一个真实的播放器**，因此挖矿器会通过 docker socket 运行一个 Chrome 边车容器 —— 这就使得 **Kick 必须依赖 Docker**。

> **树莓派 / ARM：** 挖矿器镜像同时为 `linux/amd64` *和*
> `linux/arm64` 发布，所以 Twitch 掉宝可以在树莓派上原生运行。Kick 边车容器
> 目前**仅支持 amd64**：Linux 版的 Google Chrome 仅有 x86-64 版本，而边车容器
> 专门捆绑它，是为了获得用于解码 Kick IVS 流的专有 **H.264/AAC 编解码器** ——
> arm64 上的开源 Chromium 并不包含这些编解码器。所以目前
> Kick-on-ARM 需要一台 amd64 主机；一条无浏览器的 Kick 路径（实现后即可解除此限制）正在探索中。
...
|---|---|---|
| Linux `x86-64` | ✅ | ✅ |
| Linux `arm64` / 树莓派 | ✅ | ❌ — 边车容器仅支持 amd64 |
| macOS / Windows · Docker Desktop（Intel） | ✅ | ✅ |
| macOS / Windows · Apple Silicon | ✅ | ❌ — amd64 边车容器 |
| 从源码 `go build`（任意操作系统） | ✅ | 需要 Docker + 一台 amd64 主机 |
